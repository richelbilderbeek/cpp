 ...
 * Do use a non-[pointer](CppPointer.md) [reference](CppReference.md) object for expensive-to-copy data types, like [std::string](CppStdString.md), [std::vector](CppStdVector.md)&lt;int&gt; or Database. Make the argument [const](CppConst.md) if it must be marked read-only [10]
